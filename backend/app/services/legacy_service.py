...
async def _consume_global_force_refresh_slot() -> bool:
	...
	async with global_force_refresh_lock:
		now = utc_now()
		if (
			runtime_state.last_global_force_refresh_at is not None
			and now - _coerce_utc_datetime(runtime_state.last_global_force_refresh_at)
			< GLOBAL_FORCE_REFRESH_INTERVAL
		):
			return False

		runtime_state.last_global_force_refresh_at = now
		return True
